<template>
        ...
        <div class="col-sm-4 col-xs-11">
            <q-input :model-value="toDate" @update:model-value="onToDateChange" mask="####-##-## ##:##"
                ...
                <template v-slot:prepend>
                    <q-icon name="event" class="cursor-pointer">
                        <q-popup-proxy cover transition-show="scale" transition-hide="scale">
                            <q-date :model-value="toDate" @update:model-value="onToDateChange" mask="YYYY-MM-DD HH:mm">
                                <div class="row items-center justify-end">
                                    <q-btn v-close-popup label="Close" color="primary" flat />
                                </div>
                            </q-date>
                ...
                <template v-slot:append>
                    <q-icon name="access_time" class="cursor-pointer">
                        <q-popup-proxy cover transition-show="scale" transition-hide="scale">
                            <q-time model-value="toDate" @update:model-value="onToDateChange" mask="YYYY-MM-DD HH:mm"
                                format24h>
                                <div class="row items-center justify-end">
                                    <q-btn v-close-popup label="Close" color="primary" flat />
                                </div>
    ...
    <q-pagination ref="qpagination" v-if="activities" v-model="activities.page" :max="activities.pages" direction-links
        @update:model-value="loadMore" class="q-pl-sm q-pt-md q-pb-md">
    </q-pagination>
